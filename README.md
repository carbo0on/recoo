...
يُنشأ تلقائيًا في نهاية كل تشغيل: `report.html` — صفحة واحدة **تفاعلية** (dark theme)
بتبويبات (live hosts / DNS / ports / URLs / params / injection / findings / screenshots /
tools)، بحث/فلترة فوري داخل كل قسم، تلوين خطورة nuclei، ومعرض سكرين شوت بنافذة تكبير
(lightbox) — كلها مضمّنة في ملف واحد بلا أصول خارجية. لإعادة توليده من نتائج موجودة:
`--report-only`. لتعطيله: `--no-html`.

### المرونة والإصلاح التلقائي / Resilience & self-healing

recoo يكمل التشغيل بدل التوقف عند المشاكل البسيطة:

- **قوائم الكلمات**: إن لم يتوفّر الحجم المطلوب (مثلاً `full`) ينتقل تلقائيًا لأصغر حجم
  موجود (`short` ثم `micro`) بدل تخطّي الأداة.
- **كشف httpx الصحيح**: يتحقّق recoo أن `httpx` على الـ PATH هو فعلاً أداة
  ProjectDiscovery وليس عميل بايثون `httpx` المشهور (يحمل نفس الاسم). إن وُجدت النسخة
  الصحيحة في مكان آخر (مثل `~/go/bin`) يستخدمها تلقائيًا، وإلا يتخطّى httpx برسالة علاج
  واضحة بدل إنتاج مخرجات فاسدة.
- **`live.txt`**: إن لم يُنتج httpx مضيفات حيّة صالحة (مثلاً عند تثبيت نسخة `httpx`
  الخاطئة)، تُشتقّ القائمة من المضيفات المحلولة عبر `https://` كي تستمر مراحل اللقطات
  والزحف والـ params وnuclei.
- **تصنيف الفشل**: الأداة التي تُنتج مخرجات لا تُعلَّم كفاشلة لمجرّد رمز خروج ≠ 0، ويُحفظ
  خرج الأخطاء في `<output>/.recoo/logs/<tool>.log` لتشخيص الإخفاقات الحقيقية.
